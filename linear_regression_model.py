...
class VectorComparisonModel:
    def __init__(self, testuser=None, tfidf=True, ignore=tuple(), limit=0, sentiment=None, rts=True, n_comps=75):
        self.rts = rts
        ...
        if testuser is None:
            ...
            self.training_users = [x for x in ground_truths_politicians.keys() if x not in ignore + (testuser,)]
            self.ytrain = np.array([list(ground_truths_politicians[x]) for x in self.training_users])
            try:
    ...
    def createvectors(self, training, testing, pca=True, topic_model=False, n_topics=8, lsi=False):
        ...
        training['tfidf_vector'] = list(self.vectorizer.transform(training['cleaned']).toarray())
