...
def plotrunNolans(df, testuser):
    ...
    df2 = pd.DataFrame.from_dict(
        {testuser: ground_truths_politicians[testuser]}, orient="index"
    )
    df2 = df2.reset_index()
    ...
    ax = sns.scatterplot(
        x="e_score", y="s_score", style="Type", hue="Type", data=df1, s=200
    )

    ax.set(ylabel="Ground Truth Social %", xlabel="Ground Truth Economic %")
    ax.set_title(
        "{}'s Political Affiliation Estimation via Linear Regression over By-User Feature Matrix".format(
            testuser
        )
    )
    plt.ylim(-0.1, 1.1)
...
def datasetviz():
    ...
    df = pd.read_sql(
        """select user_id, author_handle, e_score, s_score, real_party, score_freq from user;""",
        conn,
        index_col="user_id",
    )
...
def plotrunNolans2(df):
    ...
    ax = sns.scatterplot(
        x="e_score", y="s_score", style="Failed", hue="Failed", data=df, s=120
    )
    ...
    for line in range(0, df.shape[0]):
        if df.author_handle[line] not in ground_truths_politicians:
            loc = [df.e_score[line] + 0.002, df.s_score[line] + 0.005]
