...
class LDA:
    ...
    def preprocess(self, text):
        ...
        for word in (
            ignore
            + [x for x in ground_truths_pundits.keys()]
    ...
    def get_corpus(self, rts=True, ignore=(), sentiment=None, limit=0, tfidf=True):
        ...
        sql = "select id, tweet, author_handle, compound_score from tweet_text where author_handle in {}".format(
            str(tuple(self.users))
        )
    ...
    def getScores(self, corpus, df, num_topics=10):
        ...
        for user in self.users:
            ...
            best_score_df = pd.DataFrame(
                [
                    sorted(self.LDA[vec], key=lambda tup: -1 * tup[1])[0]
            ...
            topic_sentiment_scores = [
                (
                    ...
                    * user_df[user_df["best_topic"] == topic].loc[:, "best_score"]
                    / user_df[user_df["best_topic"] == topic]
                    .loc[:, "best_score"]
                    .sum(axis=0)
                ).sum(axis=0)
    ...
    def get_nolan_scores(self, testuser, scores, div_number):
        ...
        test_df["tweet"] = test_df["tweet"].map(
            self.preprocess
        )  # tokenize + lemmatize + stem
        user_vector = [self.dictionary.doc2bow(tweet) for tweet in test_df["tweet"]]
        best_score_df = pd.DataFrame(
            [sorted(self.LDA[vec], key=lambda tup: tup[1])[0] for vec in user_vector],
            index=test_df.index,
        )
        ...
        test_scores = [
            (
                ...
                * test_df[test_df["best_topic"] == topic].loc[:, "best_score"]
                / test_df[test_df["best_topic"] == topic]
                .loc[:, "best_score"]
                .sum(axis=0)
            ).sum(axis=0)
        ...
        error = (((s_true - s_score) ** 2) + ((e_true - e_score) ** 2)) ** 0.5

        return [testuser, self.cv_id, s_score, e_score, error]


def batch_test(
    cv_id, sentiment, limit, ignore, rts, num_topics, tfidf, blind, div_number
):
    ...
    if blind is False:
        corpus, df = model.get_corpus(
            rts=rts, ignore=ignore, sentiment=sentiment, limit=limit, tfidf=tfidf
        )
        ...
        for user in ground_truths_pundits.keys():
            ...
            nolan_scores = model.get_nolan_scores(
                div_number=div_number, testuser=user, scores=scores
            )
        ...
        for user in ground_truths_pundits.keys():
            ...
            nolan_scores = model.get_nolan_scores(
                testuser=user, scores=scores_dict, div_number=div_number
            )
            results.append(nolan_scores)
