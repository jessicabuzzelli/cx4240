...
class LDA:
    ...
    def getScores(self, corpus, df, num_topics=10):
        ...
        ## Test each training user against the LDA and get their scores
        ...
        for user in self.users:
            ...
            best_score_df = pd.DataFrame([sorted(self.LDA[vec], key=lambda tup: -1*tup[1])[0] for vec in user_vector],
                                         index=user_df.index) # TODO - see if [-1] makes more sense
...
def getTopics():
    ...
    combos = [(263, 48, 'LDAsentiment', False, True, True, 0, None, 4, tuple(), 10),
              (263, 48, 'LDAsentiment', False, True, True, 0, None, 6, tuple(), 10),
              (263, 48, 'LDAsentiment', False, True, True, 0, None, 8, tuple(), 10),
              (263, 48, 'LDAsentiment', False, True, True, 0, None, 10, tuple(), 10)]  # mirrors 108

    model = LDA(cv_id=0)
    corpus, df = model.getCorpus(RTs=True, ignore=tuple(), sentiment=None, limit=0, tfidf=True)

    for cv_id, run_id, type, blind, tfidf, rts, user_limit, sentiment, num_topics, ignored_users, div_number in combos:
        #results = runCV(cv_id, sentiment, user_limit, ignored_users, rts, num_topics, tfidf, blind, div_number)

        LDAmodel = LdaModel(corpus=corpus, id2word=model.dictionary, num_topics=num_topics,
                            update_every=1, passes=2)
        ...
        user_df = df[df['author_handle'] == user]
        user_vector = [model.dictionary.doc2bow(tweet) for tweet in user_df['tweet']]
        best_score_df = pd.DataFrame([sorted(LDAmodel[vec], key=lambda tup: -1*tup[1])[0] for vec in user_vector],
                                     index=user_df.index)
        user_df.loc[:,'best_topic'] = best_score_df.loc[:,0]
        user_df.loc[:,'best_score'] = best_score_df.loc[:,1]
        topic_sentiment_scores = [(
                                  ...
                                  for topic in range(num_topics)]
        print(topic_sentiment_scores)

        LSImodel = LsiModel(corpus=corpus, id2word=model.dictionary, num_topics=num_topics)
        print(LSImodel.print_topics(num_topics=num_topics, num_words=15))
        ...
        for vec in user_vector:
            try:
                top_topic, top_score = sorted(LSImodel[vec], key=lambda tup: tup[1])[-1]
